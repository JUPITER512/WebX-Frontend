...
import { forgetPasswordEmail } from "../../../app/store.js";
import { useNavigate, Link, createSearchParams } from "react-router-dom";

function Signin() {
  const navigate = useNavigate()
  const [email, setEmail] = useRecoilState(forgetPasswordEmail);
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/v1/user/getOtp', data);
      if (response.status == 200) {
        navigate(`/otp?${createSearchParams({email:email})}`)
      }
      setEmail('');
    } catch (error) {
  ...
  return (
    <Index>
      <div className="border-[1px] border-gray-500 rounded-lg dark:border-gray-300 w-full lg:w-1/2 bg-white dark:bg-gray-800 m-auto min-h-[40rem] px-8 py-10 font-Playwrite shadow-[0_0px_20px_10px_rgba(0,0,0,0.2)] dark:shadow-[0_0px_20px_20px_rgba(255,255,255,0.1)]">
        <h1 className="text-center my-4 font-semibold text-2xl dark:text-white text-black">Signup Here</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 text-center">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-center">
            ...
            <Input id="email" type="email" classes="w-2/3 md:w-1/3 rounded-md bg-gray-300 dark:bg-gray-300 antialiased px-2 py-1 bg-opacity-50 border-[1px] border-gray-800" recoilAtom={forgetPasswordEmail} />
          </div>
        ...
        <p className="text-black dark:text-white text-center mt-4">Do you Remember your password? Then <Link className=" underline hover:text-blue-700 duration-200 transition-colors" to={'/Sign-in'}>Sign In</Link> </p>
      </div>
    </Index>
  )
}
